...
function [fh, fh_idv, stat] = run_show_kernel_orthCoh(cond_switch, morph_level, cond, fluc, resp, subj, opt)

opt.medRT = round(prctile(cellfun(@(x) size(x,1), fluc), 60));

[coh_task1, coh_task2, coh_main, coh_orth] = get_coh(morph_level, cond);

for s = 1:length(opt.subj_list)
    k = ones(6,1);
    I_subj = strcmp(subj, opt.subj_list{s});
    fluc(cond==1 & I_subj) = cellfun(@(x) x(:,4:6)*k(4:6)/sum(k(4:6)), fluc(cond==1 & I_subj), 'uni', 0);
    fluc(cond==2 & I_subj) = cellfun(@(x) x(:,1:3)*k(1:3)/sum(k(1:3)), fluc(cond==2 & I_subj), 'uni', 0);
end
coh = coh_orth;
...
for s = 1:nsubj
    ...
    [fh_idv{s}, stat{s}] = show_kernel(cond_switch(I), coh(I), fluc(I), resp(I), opt);
end
...
if opt.plot_idv_fh
    fh = figure('color', 'w', 'position', [100 100 200 100*nfeature]);
    ax = nan(nfeature,2);
    for f = 1:nfeature
        ax(f,1) = subplot(nfeature,2,2*f-1);
        hold on;
